...
export function DurgaSaptashatiReader({ initialPage }: { initialPage: number }) {
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        ...
        {leafPage.kind === "leaf" ? (
          <Button
            ...
            variant="outline"
            onClick={listen}
            data-testid="durga-saptashati-listen"
          >
        ...
        {leafPage.kind === "cover" ? (
          <div className="space-y-6 px-6 py-10 text-center sm:px-10">
            <Image
              ...
              priority
            />
            <h1 className="font-serif text-3xl">॥ दुर्गा सप्तशती ॥</h1>
            <p className="text-sm leading-7 text-muted-foreground">
              Devi Mahatmya katha-sar for Chandi patha — Kavacha, Argala, Kilaka, then thirteen
              adhyayas.
            </p>
            <Button asChild>
              <Link href={bookPath(3)}>Start Kavacham</Link>
            </Button>
          </div>
        ) : null}
        ...
        {leafPage.kind === "leaf" ? (
          <div className="px-5 py-8 sm:px-8">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">
              {charitraLabel(leafPage.leaf.charitra)} · {leafPage.leaf.kathaTitleMr}
            </p>
            <h1 className="mt-3 font-devanagari text-3xl">
              {leafPage.leaf.kind === "adhyay" && leafPage.leaf.adhyay !== null
                ? `॥ अध्याय ${toDevanagariNumeral(leafPage.leaf.adhyay)} ॥`
                : `॥ ${leafPage.leaf.titleMr} ॥`}
            </h1>
            ...
            <p className="theme-note mt-8 text-xs leading-5">
              Tradition — original Hind AI katha-sar on Markandeya Purana Devi Mahatmya motifs. Not
              a publisher patha scan.
            </p>
          </div>
        ) : null}
      </article>
