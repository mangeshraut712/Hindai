...
test.describe("Festival encyclopedia", () => {
  test("festivals hub and deep Maha Shivaratri article", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /Hindu festivals with origin/i })
    ).toBeVisible();
    ...
    await page.getByRole("link", { name: /Maha Shivaratri/i }).first().click();
...
test.describe("Katha grantha", () => {
  test("katha hub and Mahadeva chapters", async ({ page }) => {
    ...
    await page.getByRole("link", { name: /Mahadeva Katha/i }).first().click();
